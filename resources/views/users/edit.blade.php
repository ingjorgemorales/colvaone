            ...
            <form method="POST" action="{{ route('users.update', $user) }}" style="display:flex;flex-direction:column;gap:16px">
                ...
                <div style="display:grid;gap:16px;grid-template-columns:1fr 1fr">
                    ...
                    <div>
                        ...
                        <input name="document_number" type="text" value="{{ old('document_number', $user->document_number) }}" class="input-field">
                ...
                <div>
                    <label style="display:flex;align-items:center;gap:10px;cursor:pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $user->is_active) ? 'checked' : '' }} style="accent-color:#123f6e;width:16px;height:16px">
                        <span style="font-size:13px;font-weight:500;color:#475569">Usuario activo</span>
                    </label>
                    @error('is_active') <p style="font-size:12px;color:#dc2626;margin-top:4px">{{ $message }}</p> @enderror
                </div>
