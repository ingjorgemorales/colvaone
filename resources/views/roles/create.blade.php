<x-layouts.app title="Crear rol | {{ config('app.name') }}" heading="Crear rol" subheading="Define un nuevo rol y sus permisos">
    <div style="max-width:800px;margin:0 auto">
        <div class="card" style="padding:24px">
            <form method="POST" action="{{ route('roles.store') }}" style="display:flex;flex-direction:column;gap:16px">
                @csrf

                <div style="display:grid;gap:16px;grid-template-columns:1fr 1fr">
                    <div>
                        <label style="display:block;font-size:13px;font-weight:500;color:#475569;margin-bottom:6px">Nombre del rol *</label>
                        <input name="name" type="text" value="{{ old('name') }}" required class="input-field" placeholder="Ej: Supervisor">
                    ...
                    <div>
                        ...
                        <input name="description" type="text" value="{{ old('description') }}" class="input-field" placeholder="Descripcion del rol">
                    </div>
                </div>
                            ...
                            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:8px">
                                @foreach($group['items'] as $perm => $label)
                                    <label style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:8px;font-size:13px;color:#475569;cursor:pointer;transition:background 0.2s" onmouseover="this.style.background='rgba(18,63,110,0.04)'" onmouseout="this.style.background='transparent'">
                                        <input type="checkbox" name="permissions[]" value="{{ $perm }}" {{ in_array($perm, old('permissions', [])) ? 'checked' : '' }} style="accent-color:#123f6e;width:16px;height:16px">
                                        {{ $label }}
                                    </label>
                                @endforeach
                            </div>
                ...
                <div style="display:flex;align-items:center;gap:12px;padding-top:8px">
                    <button type="submit" class="btn-primary">
                        <i data-lucide="shield-plus" style="width:16px;height:16px"></i> Crear rol
                    </button>
